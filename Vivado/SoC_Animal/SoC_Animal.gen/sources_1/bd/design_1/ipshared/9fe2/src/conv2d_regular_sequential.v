module conv2d_regular_sequential #(
    ...
    parameter integer WEIGHT_SIZE = (OUT_C * IN_C * K * K)
) (
    input wire clk,
    input wire rst,
    input wire start,
    ...
    always @(*) begin
        ...
        last_mac = (ic == IN_C - 1 && ky == K - 1 && kx == K - 1);
    end
            ...
            case (state)
                ...
                S_MAC: begin
                    ...
                    if (pipe_valid_dd && pipe_last_dd) begin
                        ...
                        weight_addr <= oc * IN_C * K * K + ic * K * K + ky * K + kx;
                        if (in_range) begin
                            input_addr <= ic * IN_H * IN_W + iy * IN_W + ix;
                        end
                        pipe_valid <= 1'b1;
